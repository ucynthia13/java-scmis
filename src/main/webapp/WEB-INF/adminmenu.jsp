<li class="dropdown"><a class="idTabs dropdown-toggle" data-toggle="dropdown" href="#admin" class="border border-dark"> Admin</a>
	<ul class="noJS dropdown-menu">

		<li><a href="createuser.php?page=createuser"> Create User</a></li>

		<li><a href="listuser.php?page=users&&action=list"> List of users</a></li>

	</ul></li>
<li class="dropdown"><a
	href="listemployees.php?page=employees&&user_role=adm&&action=list"> Employees</a></li>

<li class="dropdown"><a class="idTabs dropdown-toggle" data-toggle="dropdown" href="#students"> Students</a>
	<ul class="noJS dropdown-menu">
		<li><a
			href="liststudents.php?page=students&&user_role=adm&&action=list"> Students</a></li>
		<li><a
			href="listparents.php?page=parents&&user_role=adm&&action=list"> Parents</a></li>
	</ul></li>

<li class="dropdown"><a class="idTabs dropdown-toggle" data-toggle="dropdown" href="#marks"> Marks</a>
	<ul class="noJS dropdown-menu">
		<li><a href="listmarks.php?page=marks&&action=list"> Marks</a></li>
		<li><a
			href="listcourse.php?page=courses&&action=list"> Courses</a></li>
	</ul></li>

<li class="dropdown"><a class="idTabs dropdown-toggle" data-toggle="dropdown" href="#reports"> Reports</a>
	<ul class="noJS dropdown-menu">
		<li><a
			href="students.php?page=reportsearch&&user_role=adm&&action=search"> Students reports</a></li>
		<li><a
			href="courses.php?page=reportcourses&&user_role=adm&&action=search"> Courses reports </a></li>
		<li><a
			href="marks.php?page=markssearch&&user_role=adm&&action=search"> Marks Report</a></li>
		<li><a
			href="Studentspdf.php?"> All Students PDF</a></li>
	</ul></li>
